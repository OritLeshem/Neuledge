...
export interface SelectQuery<M extends QueryMode, I extends StateDefinition, O extends StateDefinition, R = NonNullable<unknown>> {
    /**
...
export interface SelectQueryOptions<S extends StateDefinition, P extends Select<S> = Select<S>> {
    select?: P | true | null;
}
export type Select<S extends StateDefinition> = {
    [K in keyof Merge<StateType<S>>]?: boolean;
};
export type SelectParam<S extends StateDefinition, P extends Select<S>> = Subset<P, Select<S>>;
export type QueryProjection<S extends StateDefinition> = Select<S> | true | null;
export type QueryEntity<S extends StateDefinition, P extends QueryProjection<S>, R> = (P extends Select<S> ? ProjectedEntity<S, P> : Entity<S>) & R;
