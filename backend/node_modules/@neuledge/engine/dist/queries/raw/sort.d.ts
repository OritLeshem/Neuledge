...
export type Sort<S extends StateDefinition> = SortIndex<S> | readonly SortField<S>[];
export type SortIndex<S extends StateDefinition> = SortedField<{
    [K in keyof StateIndexes<S>]: StateIndexes<S>[K]['unique'] extends true ? never : K;
}[keyof StateIndexes<S>] & string>;
export type SortField<S extends StateDefinition> = SortedField<keyof StateType<S> & string>;
