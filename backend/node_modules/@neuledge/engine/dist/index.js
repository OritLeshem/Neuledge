...
var require_es6 = __commonJS({
  "../../node_modules/fast-deep-equal/es6/index.js"(exports, module2) {
    "use strict";
    init_cjs_shims();
    module2.exports = /* @__PURE__ */ __name(function equal2(a, b) {
      if (a === b)
        return true;
...
__export(src_exports, {
  $: () => escaped_exports,
  NeuledgeEngine: () => NeuledgeEngine,
  NeuledgeError: () => NeuledgeError,
  QueryClass: () => QueryClass,
  State: () => State,
  createEitherDefintion: () => createEitherDefintion,
  createEntityList: () => createEntityList,
  createMutation: () => createMutation,
  fromSortedField: () => fromSortedField,
  isStateDefinitionScalarTypeScalar: () => isStateDefinitionScalarTypeScalar,
  isStateDefinitionScalarTypeStates: () => isStateDefinitionScalarTypeStates,
  parseSortedField: () => parseSortedField,
  resolveDefer: () => resolveDefer
});
...
// src/error.ts
init_cjs_shims();
var import_store = require("@neuledge/store");
...
var _NeuledgeError = class extends Error {
  ...
  static fromError(originalError, code, message, hideOriginalError) {
    ...
    const error = new _NeuledgeError(originalError instanceof import_store.StoreError ? fromStoreErrorCode(originalError.code, code) : code ?? _NeuledgeError.Code.INTERNAL_ERROR, hideOriginalError ? message ?? "An unknown error occurred" : message == null ? orgMsg : `${message}: ${orgMsg}`);
...
var fromStoreErrorCode = /* @__PURE__ */ __name((code, defaultCode) => {
  switch (code) {
    case import_store.StoreError.Code.INVALID_DATA: {
      return NeuledgeError.Code.CORRUPTED_METADATA;
    }
    case import_store.StoreError.Code.NOT_IMPLEMENTED: {
...
var formatCollectionName = /* @__PURE__ */ __name((words) => [
  ...words.slice(0, -1),
  import_pluralize.default.plural(words[words.length - 1])
...
var generateHash = /* @__PURE__ */ __name((payload) => (0, import_node_crypto.createHash)(HASH_ALGORITHM).update(JSON.stringify(payload)).digest().slice(0, METADATA_HASH_BYTES), "generateHash");
...
var ensureStoreCollections = /* @__PURE__ */ __name(async (store, metadata) => {
  const asyncLimit = (0, import_p_limit.default)(1);
...
var toDocument = /* @__PURE__ */ __name((metadata, collection, entity) => {
  ...
  for (const field of state.fields) {
    document2[field.name] = getEntityValue(entity, field.path);
  }
  return document2;
}, "toDocument");
...
init_cjs_shims();
var toEntityList = /* @__PURE__ */ __name((metadata, collection, list) => Object.assign(list.map((document2) => {
  const stateHash = document2[collection.reservedNames.hash];
  return Buffer.isBuffer(stateHash) ? getStateEntity(metadata, collection, stateHash, document2) : void 0;
...
var toMaybeEntity = /* @__PURE__ */ __name((metadata, collection, document2) => {
  const stateHash = document2?.[collection.reservedNames.hash];
  return Buffer.isBuffer(stateHash) ? getStateEntity(metadata, collection, stateHash, document2) : null;
}, "toMaybeEntity");
var toEntityOrThrow = /* @__PURE__ */ __name((metadata, collection, document2) => {
  const stateHash = document2?.[collection.reservedNames.hash];
  if (!Buffer.isBuffer(stateHash)) {
    throw new NeuledgeError(NeuledgeError.Code.DOCUMENT_NOT_FOUND, "Document not found");
  }
  return getStateEntity(metadata, collection, stateHash, document2);
}, "toEntityOrThrow");
var getStateEntity = /* @__PURE__ */ __name((metadata, collection, stateHash, document2) => {
  ...
  const entity = {
    $state: state.name,
    $version: document2[collection.reservedNames.version] ?? 0
  };
  for (const field of state.fields) {
    if (!(field.name in document2) || !field.path)
      continue;
    const rawValue = document2[field.name];
  ...
  for (const relation of state.relations) {
    if (!relation.path)
      continue;
    const childDocument = document2[relation.name];
...
var execInitMany = /* @__PURE__ */ __name(async (engine, options) => {
  ...
  const updatedEntities = documents.map((oldDoc, i) => {
    ...
    const entity = toEntityOrThrow(metadata, collection, document2);
    return {
      entity,
      document: document2,
      oldEntity: null
...
var execFindUnique = /* @__PURE__ */ __name(async (engine, options) => {
  const metadata = await engine.metadata;
  const { states, collection } = chooseStatesCollection(metadata, options.states);
  const [document2] = await engine.store.find({
    collection,
    ...convertSelectQuery(collection, options),
    ...convertExpandQuery(metadata, collection, options),
    ...convertPopulateOneQuery(metadata, collection, options),
  ...
  return toMaybeEntity(metadata, collection, document2);
}, "execFindUnique");
var execFindUniqueOrThrow = /* @__PURE__ */ __name(async (engine, options) => {
  ...
  return toEntityOrThrow(metadata, collection, document2);
...
var alterEntityList = /* @__PURE__ */ __name(async (states, entities, method, args) => {
  const asyncLimit = (0, import_p_limit3.default)(10);
...
init_cjs_shims();
var import_p_limit4 = __toESM(require("p-limit"));
var alterStoreDocuments = /* @__PURE__ */ __name(async (store, collection, documents, updates) => {
  const asyncLimit = (0, import_p_limit4.default)(10);
  const deleteDocuments2 = [];
  const res = await Promise.all(documents.map((document2, index) => {
    const updated = updates[index];
    if (updated == null) {
      deleteDocuments2.push(document2);
      return true;
    }
    return asyncLimit(() => updateStoreDocument(store, collection, document2, updated));
  }));
...
var updateStoreDocument = /* @__PURE__ */ __name(async (store, collection, document2, updated) => {
  const setEntries = Object.entries(updated).filter(([key, value]) => value !== document2[key]);
  if (!setEntries.length) {
    return true;
  }
  const res = await store.update({
    collection,
    where: getWhereRecordByPrimaryKeys(collection, document2),
...
var getWhereByPrimaryKeys = /* @__PURE__ */ __name((collection, documents) => ({
  $or: documents.map((document2) => getWhereRecordByPrimaryKeys(collection, document2))
}), "getWhereByPrimaryKeys");
var getWhereRecordByPrimaryKeys = /* @__PURE__ */ __name((collection, document2) => Object.fromEntries([
  ...Object.keys(collection.primaryKey.fields).map((key) => [
    key,
    {
      $eq: document2[key] ?? null
    }
  ]),
  [
    collection.reservedNames.hash,
    {
      $eq: document2[collection.reservedNames.hash] ?? null
    }
  ],
  [
    collection.reservedNames.version,
    {
      $eq: document2[collection.reservedNames.version] ?? 0
    }
  ]
...
var execAlterMany = /* @__PURE__ */ __name(async (engine, options) => {
  ...
  for (let retries = 1; retries < ALTER_VERSION_RETRIES; retries++) {
    ...
    if (ctx.entities.size < documents.length) {
      ...
      leftDocs = documents.filter((document2) => !ctx.entities.has(getDocumentKey(ctx.collection, document2)));
    }
    if (!leftDocs?.length) {
...
var alterDocuments = /* @__PURE__ */ __name(async (ctx, documents) => {
  ...
  for (const [i, flag] of success.entries()) {
    if (!flag)
      continue;
    const document2 = updatedDocs[i] ?? documents[i];
    const entity = updated[i] ?? entities[i];
    const key = getDocumentKey(ctx.collection, document2);
...
var getDocumentKey = /* @__PURE__ */ __name((collection, document2) => JSON.stringify(Object.keys(collection.primaryKey.fields).map((key) => document2[key] ?? null)), "getDocumentKey");
...
__export(escaped_exports, {
  ...
  runtime: () => import_scalars.runtime,
  scalars: () => import_scalars.types
});
init_cjs_shims();
